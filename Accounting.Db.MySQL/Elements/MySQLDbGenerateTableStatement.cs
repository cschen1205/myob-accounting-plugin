...
{
    public class MySQLDbGenerateTableStatement : DbGenerateTableStatement
    {
            ...
            foreach (string fieldname in mFields.Keys)
            {
                ...
                if (mPrimaryKeys.ContainsKey(fieldname))
                {
                    ...
                    if (fieldtype == DbManager.FIELDTYPE.INTEGER_INDEXED)
                    {
                        ...
                        sb.AppendFormat("{0}  INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT", mDbMgr.FieldAlias(fieldname));
                    }
                    else
                    {
