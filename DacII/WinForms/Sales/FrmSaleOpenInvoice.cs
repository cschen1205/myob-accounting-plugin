...
namespace DacII.WinForms.Sales
{
    ...
    public partial class FrmSaleOpenInvoice : BaseView
    {
        private BOSaleOpenInvoice mModel;
        private BOViewModel mViewModel;
       
        public FrmSaleOpenInvoice(ApplicationPresenter ap, BOSaleOpenInvoice model)
            : base(ap)
        {
            InitializeComponent();
        ...
        private void Record(object sender, EventArgs args)
        {
            if (mViewModel.ValidateModel())
            {
                ...
                {
                    MessageBox.Show(result.Error, "Update Sale Open Invoice Failed");
                    DialogResult = DialogResult.None;
                }
                else if (result.Status == OpResult.ResultStatus.CreateFailedOnCriteria)
                {
                    MessageBox.Show(result.Error, "Create Sale Open Invoice Failed");
                    DialogResult = DialogResult.None;
                }
                else
                {
                    ...
                    {
                        MessageBox.Show("Sale Open Invoice Recorded!");
                    }
                    else
                    {
