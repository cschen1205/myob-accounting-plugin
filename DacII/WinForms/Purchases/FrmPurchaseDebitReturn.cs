...
namespace DacII.WinForms.Purchases
{
    ...
    public partial class FrmPurchaseDebitReturn : BaseView
    {
        private BOPurchaseDebitReturn mModel;
        private BOViewModel mViewModel;

        public FrmPurchaseDebitReturn(ApplicationPresenter ap, BOPurchaseDebitReturn model)
            : base(ap)
        {
            InitializeComponent();
        ...
        public BOPurchaseDebitReturn Model
        {
            set
            {
                mModel = value;
        ...
        private void Record(object sender, EventArgs args)
        {
            if (mViewModel.ValidateModel())
            {
                ...
                {
                    MessageBox.Show(result.Error, "Update Purchase Debit Return Failed");
                    DialogResult = DialogResult.None;
                }
                else if (result.Status == OpResult.ResultStatus.CreateFailedOnCriteria)
                {
                    MessageBox.Show(result.Error, "Create Purchase Debit Return Failed");
                    DialogResult = DialogResult.None;
                }
                else
                {
                    ...
                    {
                        MessageBox.Show("Purchase Debit Return Recorded!");
                    }
                    else
                    {
