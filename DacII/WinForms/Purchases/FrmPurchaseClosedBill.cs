...
namespace DacII.WinForms.Purchases
{
    ...
    public partial class FrmPurchaseClosedBill : BaseView
    {
        private BOPurchaseClosedBill mModel;
        private BOViewModel mViewModel;

        public FrmPurchaseClosedBill(ApplicationPresenter ap, BOPurchaseClosedBill model)
            : base(ap)
        {
            InitializeComponent();
        ...
        public BOPurchaseClosedBill Model
        {
            set
            {
                mModel = value;
        ...
        private void Record(object sender, EventArgs args)
        {
            if (mViewModel.ValidateModel())
            {
                ...
                {
                    MessageBox.Show(result.Error, "Update Purchase Closed Bill Failed");
                    DialogResult = DialogResult.None;
                }
                else if (result.Status == OpResult.ResultStatus.CreateFailedOnCriteria)
                {
                    MessageBox.Show(result.Error, "Create Purchase Closed Bill Failed");
                    DialogResult = DialogResult.None;
                }
                else
                {
                    ...
                    {
                        MessageBox.Show("Purchase Closed Bill Recorded!");
                    }
                    else
                    {
