...
namespace DacII.WinForms.Purchases
{
    ...
    public partial class FrmPurchaseOpenBill : BaseView
    {
        private BOPurchaseOpenBill mModel;
        private BOViewModel mViewModel;

        public FrmPurchaseOpenBill(ApplicationPresenter ap, BOPurchaseOpenBill model)
            : base(ap)
        {
            InitializeComponent();
        ...
        private void Record(object sender, EventArgs args)
        {
            if (mViewModel.ValidateModel())
            {
                ...
                {
                    MessageBox.Show(result.Error, "Update Purchase Open Bill Failed");
                    DialogResult = DialogResult.None;
                }
                else if (result.Status == OpResult.ResultStatus.CreateFailedOnCriteria)
                {
                    MessageBox.Show(result.Error, "Create Purchase Open Bill Failed");
                    DialogResult = DialogResult.None;
                }
                else
                {
                    ...
                    {
                        MessageBox.Show("Purchase Open Bill Recorded!");
                    }
                    else
                    {
