...
namespace DacII.WinForms.Purchases
{
    ...
    public partial class FrmPurchaseOrder : BaseView
    {
        private BOPurchaseOrder mModel = null;
        private BOViewModel mViewModel;

        public FrmPurchaseOrder(ApplicationPresenter ap, BOPurchaseOrder model)
            : base(ap)
        {
            InitializeComponent();
        ...
        private void Record(object sender, EventArgs args)
        {
            if (mViewModel.ValidateModel())
            {
                ...
                {
                    MessageBox.Show(result.Error, "Update Purchase Order Failed");
                    DialogResult = DialogResult.None;
                }
                else if (result.Status == OpResult.ResultStatus.CreateFailedOnCriteria)
                {
                    MessageBox.Show(result.Error, "Create Purchase Order Failed");
                    DialogResult = DialogResult.None;
                }
                else
                {
                    ...
                    {
                        MessageBox.Show("Purchase Order Recorded!");
                    }
                    else
                    {
